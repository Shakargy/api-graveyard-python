...
def patch(batcher, options: dict) -> None:
    global _patched, _original_urlopen, _original_https_urlopen, _batcher, _service_name, _environment, _IGNORED_HOSTNAMES
    ...
    def _make_patched(original):
        def patched_urlopen(pool, method, url, **kwargs):
            ...
            if pool.host in _IGNORED_HOSTNAMES:
                return original(pool, method, url, **kwargs)
            ...
            try:
                ...
                try:
                    ...
                    pass
        return patched_urlopen

    # Patch both pools explicitly — in urllib3 v2 HTTPSConnectionPool
    # defines its own urlopen and does not inherit the HTTP patch.
    urllib3.HTTPConnectionPool.urlopen = _make_patched(_original_urlopen)
    urllib3.HTTPSConnectionPool.urlopen = _make_patched(_original_https_urlopen)
    _patched = True
